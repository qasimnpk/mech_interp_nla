"""27B variant (paths swapped 7b->27b, otherwise identical). Mechanical deletion generator: uses each atom's exact, already-validated av_spans to remove that claim's text from
...
def load_atoms(pid: int) -> list[dict]:
    return [json.loads(l) for l in open(HERE / f"tasks/02_atoms_27b_doc{pid:03d}.jsonl")]
...
def main():
    ...
    tasks_by_pid = {}
    for f in sorted((HERE / "tasks").glob("02_atomic_tasks_27b_batch*.json")):
        for t in json.loads(f.read_text()):
            tasks_by_pid[t["pilot_id"]] = t

    total = 0
    for pid in range(a.start, a.end):
        outp = HERE / f"tasks/04_rewrites_27b_doc{pid:03d}.jsonl"
