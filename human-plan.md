...
(4) The write-up structure is now four findings (§1), not three.
...
- [ ] **Re-estimate hours honestly.** As of 2026-09-08 13:55 you recorded 12 h used / 4 h remaining. Add: round-4 design
  and advisor review (evening of 09-08), the desk discussion (09-08 → 09-09), any reading of MORNING4. Hours used now
  `_fill_`; permitted remaining `_fill_` (of 16, hard max 20) + 2 h for the exec summary. Protected sleep window `_fill_`.
- [ ] **Route.** With ≤ 4–6 h of counted time left the **Compressed** route is the realistic one: §0 → §1 rough answers and
...
- [ ] **(e) Round-4 blind sheets — 35 min [AI → YOU], do first.** In this order, stop at the cap:
  ...
  3. `b1_review_blind.csv`, all 8 rows: confirm the explanations describe only "The record ends here." (finding 2).
...
- [ ] **(b) Re-derive the central results from raw data — 30 min [YOU].** Paste each line and result into `notes/human_log.md`.
  ...
  # B1: pair similarity and omission (expect cos 0.9927; asserting sentences 0 of 192)
