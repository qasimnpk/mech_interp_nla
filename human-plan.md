...
| positioning             | `notes/response_to_reframing_2026-09-07.md` (Dingeto verified, same checkpoint), `notes/novelty_vs_nla_paper.md` (§E = the framing decision of 2026-09-08: the project **is** the paper's weak-verifier experiment run controlled on the released checkpoint; §F = the pending setup-comparability audit); the preprint text is in `notes/dingeto_2026_train_the_model_text.txt` |
...
- [x] Open the live application form and confirm: deadline and timezone, required questions and limits, document format
  ```
  and sharing, AI-use disclosure, whether submissions can be edited. Do not rely on the FAQ copy in `notes/` alone.
  ```
- [x] Fill in: deadline _Sep 11, Friday evening_; planned submission time (≥ 6 h earlier) __Thursday afternoon_; human hours already used _12h__ (reconstruct
  ```
  honestly from memory and git timestamps: rounds 1–3c planning, reviews, advisor exchanges); permitted hours
  remaining _4h_; protected sleep window _lots__.
  ```
- [x] Choose the route. **Standard** ≈ 10 h of your effort incl. review (§1–5). **Compressed** ≈ 4 h: §0 → rough answers
  ```
...
- [ ] Rough executive summary and report skeleton (official limits from §0; ≤ 600 words is the working target): question,
  ...
  **Opening, decided 2026-09-08 (§3 first row):** sentence 1 is the NLA paper's own line that the AR "is only a weak
  per-claim verifier"; sentence 2 is Neel's question from the FAQ (L721: use the AR "to measure the quality of a
  description, e.g. figuring out which claims can be removed … to help reduce hallucinations"); sentence 3 is your
  answer on the released checkpoint. The lineage must be visible in the first paragraph; do not make the reader infer it.
  ```
...
**Framing decision (2026-09-08 ~15:00 EDT, your question to the desk: "reference and build on the paper's experiments, or
keep our approach?").** It is a false choice: the current design already *is* the paper's confabulation-section experiment
(claim deletion → "weak per-claim verifier") run with controls on the checkpoint the paper released, plus tests of things the
paper only hypothesised or listed as future work. Keep the approach; make the lineage explicit. Do **not** pivot to
reproducing the paper's five prediction tasks or its steganography suite: those are training-curve results on Claude models,
impossible with final checkpoints, and would spread the remaining budget thin. Full reasoning and the paper-item → our-test
map: `notes/novelty_vs_nla_paper.md` §E.


| source                                      | what it contributes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | where                                                                                                                    |
| ------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Neel's FAQ, suggested problems L719–721** | NLAs are a named problem ("What can you do with them? Do they actually work for tasks of interest?"); key resource is Celeste's Qwen3.6-27B NLA; **"I'm particularly interested in using the activation reconstructor to measure the quality of a description, e.g. figuring out which claims can be removed and improve reconstruction accuracy to help reduce hallucinations"** — i.e. S2/S3/R2/R3/C1/X3. Your headline is the controlled check of his idea on the released 7B: the score detects relevance and wording, not the tested one-fact errors | `notes/neel_drive/mats12_admissions_procedure_faq.md` L706–721; L431–436 (negative results, "building on existing work") |
...
- [ ] Write the contribution narrowly: a controlled diagnosis and readout comparison on one released NLA, with a
  ...
  ```
- [ ] Cite the NLA paper and Dingeto **before** your own numbers on finding 2; present ours as the same test with a
  ```
  different instrument (paired same-claim edits, paraphrase null, length-matched span, cluster CIs).
  ```
- [ ] Use the paper's vocabulary where it is free: their theme / entity / detail claim taxonomy as a descriptive appendix
  ```
  recut of the 490 corruptions (no model runs; `notes/novelty_vs_nla_paper.md` §C N2); their recurrence heuristic and
  best-of-N as *named* future work, not as things you tested.
  ```
- [ ] State two weaknesses in one sentence each, in the main text: finding 2 overlaps Dingeto on the same pair; Celeste's
  ```
  27B (Neel's named key resource) does not fit this machine, so the study is on the paper's own released 7B pair.
  ```
- [ ] [AI → YOU] **Setup-comparability audit — desk draft done 2026-09-08 17:00:** `notes/setup_comparability.md`
  ```
  (mechanics identical to the released pair; E4.2 / E2 / E6.1 faithful in spirit, different in unit, edit, labels and
  metric; six methods sentences in its §5; four DESK numbers to re-derive: 17/160 eval positions below the training
  minimum of 50, A−P on pos ≥ 50 = −0.00240 [−0.00329, −0.00146] n=437, block-20 norm Q75 ≈ 120, FVE-equivalent
  conversions). Read §5 and §6, decide the two open items, paste the sentences you keep into the methods paragraph.
  Source table: `notes/NLA_paper_experiment_setup.md` (agent, 1,102 lines; use its Part 5.3 and 6.2, not the whole).
  Reviewing is counted human time; cap it at 20 min.
  ```
...
- Comparability (from `notes/setup_comparability.md` §5): our claim unit is a bullet, not an atomic claim; our edits are
substitutions against a paraphrase, not rewrite-deletions of judged claims; the paper's Claude numbers are never
numerically comparable, Dingeto's are; "FVE-equivalent" figures use the training-set denominator and are approximate;
we did not run E1, E3.3, E4.1, E4.3, E5, E6 or E7 and must not imply otherwise.
- Lineage: "the paper's own weak-verifier test, run controlled on the released checkpoint", never "the paper missed this"
or "NLAs hallucinate". The paper's transforms (shuffle, paraphrase, French) are reproduced *in miniature* (one-claim
paraphrase, reorder, truncation); do not imply you reran its evaluation suite. Their metric is FVE/MSE, ours is cosine on
the direction-only scorer (MSE = 2(1 − cos) for unit vectors); say so once in methods.
...
paragraph against `notes/evidence_table.md`; section extracts from the papers in §3; the setup-comparability table
(§3, last checkbox) once the paper-setups agent output is shared. Nothing else is queued.